...
def MathiasAlg():
##Bytt ut med Henriks versjon som ikke mellomlagrer et bilde
	global videoName
	global frameCounter
...
def asd(object):
	##Bytt med Henriks versjon som ikke tar inn filsti
	#print(object)
	#print(image_path)
...
def egenFunk():
	##TODO: Kjøre dette som en egen prosess/tråd
	global test
	while(test < 2): #while som simulerer to intervaller med filming&frames
		i = datetime.now()
		#CurrentVideo = str(i.strftime('%Y_%m_%d_%H:%M:%S:%f'))
		##^Bytt ut med ønsket tidsformat over
		CurrentVideo = "2018_02_21_09:42:30" + ".mp4"
		
		print(CurrentVideo)
		
		####os.system("mkdir /home/nvidia/Bachelor/Frames/" + CurrentVideo)
		##^ Opprett mappestruktur til kommende videoklipp.
		...
		CompletedVideoQueue.enqueue(obj) ###lage objektet her inne i steden for || dersom noe feiler under filming er det best 
		#at videoen ikke får status True.
		##^ Denne køen blir sjekket i mainProg
		#print("Filmet ferdig: ",thisObject.finished)
		
		time.sleep(1.1)# For test
		test += 1
	##_mainProg behøver ikke ligge inne i her
	mainProg()
	#Skriv ut køen
...
def mainProg():
		...
		##^ Sjekker om det ligger videoer som er klare i køen
			print("Fant video")
			
			global videoName
			global frameCounter
			video = CompletedVideoQueue.dequeue() #Alle videoer som ligger i denne er ferdig behandlet.
			##^ henter ut neste, klare video
			videoName = video.name
			##^ Benytt denne for å unngå problemer med innlegging av nye intervaller & globale navn
			print(videoName)
			...
			####TRIM FILM
			####TODO: (def?)Reset alle variabler som f.eks. blir ++'et. Ex: frameCounter
